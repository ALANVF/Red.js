...
export default class Vector_Char8 extends VectorOf<"char!", 8> {
	constructor(values: number[] | Uint8Array) {
		super("char!", 8, values);
	}
	
	setNew(values: number[]): this {
		this.repr = new Uint8Array(values);
		return this;
	}
	
	copy(): Vector_Char8 {
		return new Vector_Char8([...this.repr]);
	}
	
	copyAs(fn: (v: Uint8Array) => Uint8Array): Vector_Char8 {
		return new Vector_Char8(fn(this.repr));
	}
	
	push(...values: number[]): number[] {
		this.repr = Uint8Array.of(...this.repr, ...values);
		return values;
	}
	
	unshift(...values: number[]): number[] {
		this.repr = Uint8Array.of(...values, ...this.repr);
		return values;
	}
	...
	insert(index: number, ...values: number[]): number[] {
		if(index == 0) {
			...
			this.repr = Uint8Array.of(...this.repr.slice(0, index), ...values, ...this.repr.slice(index));
			return values;
		}
	}
	...
	remove(index: number, count: number): number[] {
		...
		if(count == 0) {
			...
			if(end >= this.repr.length) {
				...
				this.repr = Uint8Array.of(...this.repr.slice(0, index), ...this.repr.slice(end));
				return res;
			}
		}
	}
